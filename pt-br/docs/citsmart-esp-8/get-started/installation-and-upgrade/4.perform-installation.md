...
Devemos descompactar o pacote JAVA JDK no diretório /opt e crie um link
...
Execute os comandos abaixo para configuração dos pacotes para o Citsmart.
...
Agora devemos criar um usuário para administração do Wildfly.

Para isso crie o usuário **citsmart** e aplique as permissões necessárias
...
Agora devemos configurar o **PATH** do para o JAVA_HOME e o JBOSS_HOME. Para
isso faça conforme exemplo abaixo.
...
Faça um teste para validar se o Wildfly está iniciando corretamente até esse
ponto. Para isso execute os comandos abaixo.
...
Pare o Wildfly (Iniciado anteriormente) e configure o standalone.conf no
diretório \$JBOSS_HOME/bin, conforme mostrado abaixo.

| *\# mv standalone.conf standalone.dist \# vim standalone.conf*                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *if [ "x\$JBOSS_MODULES_SYSTEM_PKGS" = "x" ]; then JBOSS_MODULES_SYSTEM_PKGS="org.jboss.byteman" fi if [ "x\$JAVA_OPTS" = "x" ]; then JAVA_OPTS="\$JAVA_OPTS -Xms5200m" JAVA_OPTS="\$JAVA_OPTS -Xmx5200m" JAVA_OPTS="\$JAVA_OPTS -XX:MinHeapFreeRatio=40" JAVA_OPTS="\$JAVA_OPTS -XX:MaxHeapFreeRatio=80" JAVA_OPTS="\$JAVA_OPTS -XX:NewRatio=8" JAVA_OPTS="\$JAVA_OPTS -XX:SurvivorRatio=32" JAVA_OPTS="\$JAVA_OPTS -XX:+UseG1GC" JAVA_OPTS="\$JAVA_OPTS -XX:G1HeapRegionSize=4" JAVA_OPTS="\$JAVA_OPTS -XX:InitiatingHeapOccupancyPercent=50" JAVA_OPTS="\$JAVA_OPTS -Djava.net.preferIPv4Stack=true" JAVA_OPTS="\$JAVA_OPTS -Dorg.jboss.resolver.warning=true" JAVA_OPTS="\$JAVA_OPTS -Duser.timezone="America/Sao_Paulo"" JAVA_OPTS="\$JAVA_OPTS -Djboss.modules.system.pkgs=\$JBOSS_MODULES_SYSTEM_PKGS" JAVA_OPTS="\$JAVA_OPTS -Djava.awt.headless=true" JAVA_OPTS="\$JAVA_OPTS -Djboss.server.default.config=standalone-full-ha.xml" JAVA_OPTS="\$JAVA_OPTS -Djboss.bind.address=0.0.0.0" JAVA_OPTS="\$JAVA_OPTS -Djboss.bind.address.management=0.0.0.0" else echo "JAVA_OPTS already set in environment; overriding default settings with values: \$JAVA_OPTS" fi* |
...
jboss-cli. Para isso inicie o Wildfly em modo standalone, conecte com o
jboss-cli e execute os comandos abaixo.
...
No bash do CLI execute os comandos abaixo para criação das propriedades do
citsmart.

| */system-property=mongodb.host:add(value="mongodb.citsmart.com") /system-property=mongodb.port:add(value="27017") /system-property=mongodb.user:add(value="admin") /system-property=mongodb.password:add(value="admin") /system-property=citsmart.protocol:add(value="http") /system-property=citsmart.host:add(value="itsm.citsmart.com") /system-property=citsmart.port:add(value="8080") /system-property=citsmart.context:add(value="citsmart") /system-property=citsmart.login:add(value="citsmart.local\\\\\\consultor") /system-property=citsmart.password:add(value="senhaConsultor") /system-property=citsmart.inventory.id:add(value="citsmartinventory") /system-property=citsmart.evm.id:add(value="citsmartevm") /system-property=citsmart.evm.enable:add(value=true) /system-property=citsmart.inventory.enable:add(value=true) /system-property=rhino.scripts.directory:add(value="")* |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
1- Cria os diretórios abaixo para serem configurados nos 3 passos de instalação
web.
...
Envie os arquivos de deploy fornecidos para o servidor e mova-os para o
...
Faça o deploy do Citsmart Neuro após finalizar os passos da próxima sessão
