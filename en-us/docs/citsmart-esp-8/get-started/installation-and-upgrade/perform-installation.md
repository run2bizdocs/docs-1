...
2. Connect the jboss-cli once again and execute the command below to add the module to the standalone-full-ha.xml   
    ...
    There are eight inputs of datasource for the citsmart_db, being four for CITSmart and four for CITSmart Neuro. The user and   password is citsmartdbuser and exemplo123 respectvely created in the section "PostgreSQL Database Server".
